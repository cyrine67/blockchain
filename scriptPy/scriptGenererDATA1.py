...
# Class to represent the current system state (normal, attack, breakdown, etc.)
class SystemState:
    def __init__(self):
        self.regime = 'NORMAL_LOW'   # initial state
        self.state_counter = 0       # counter for time spent in current state
        self.params = {}             # parameters of the state

# Function to create a single data row based on parameters
def _create_row_from_params(params, block_index, anomaly_type):
    is_anomaly = 0 if anomaly_type == 'Normal' else 1  # label anomaly
    num_tx = max(1, int(random.uniform(*params['num_tx_range'])))  # number of transactions
    time_since_last_block = random.uniform(*params['time_range']) + (num_tx * params.get('time_tx_factor', 0.01))

    avg_energy = random.uniform(*params['avg_energy_range'])  # average energy
    std_dev_energy = avg_energy * random.uniform(*params['std_energy_factor_range'])  # energy variance
    avg_amount = random.uniform(*params['avg_amount_range'])  # average transaction amount
    std_dev_amount = avg_amount * random.uniform(*params['std_amount_factor_range'])  # amount variance

    if num_tx == 1:  # single transaction case
        std_dev_energy = 0.0
        std_dev_amount = 0.0

    noise_factor = 1 + random.uniform(-0.02, 0.02)  # noise added
    total_energy = avg_energy * num_tx * noise_factor
    total_amount = avg_amount * num_tx * noise_factor

    num_theft_attempts = 1 if anomaly_type == 'Theft' else 0  # theft label
    num_alerts = 1 if is_anomaly else random.choice([0, 0, 0, 1])  # alert flag

    # Sender/receiver simulation depending on anomaly type
    if num_tx > 1:
        if anomaly_type == 'DoS':
    ...
    if unique_senders > 0:
        if anomaly_type == 'DoS':  # concentrated sender activity
    ...
    if num_tx == 1:  # one sender case
        sender_concentration = 1.0

    tx_rate = num_tx / time_since_last_block if time_since_last_block > 0.01 else 0  # transaction rate

    # Deliveries and payments simulation
...
# Function to generate the dataset and save it to CSV
def generate_lstm_ready_data(num_lines=50000):
    ...
    with open(r'C:\Users\Cyrinechalghoumi\Desktop\blockchain\data1.csv', mode='w', newline='') as file:
        ...
        for i in range(1, num_lines + 1):
            ...
            # State transitions (normal <-> anomaly)
...
if __name__ == '__main__':
    generate_lstm_ready_data(num_lines=50000)  # generate dataset
